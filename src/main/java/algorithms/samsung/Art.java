...
public class Art {
  ...
  static List<Integer> typeInfo;
  public static boolean isInRange(int y, int x) {
    return y < n && y >= 0 && x < n && x >= 0;
  }

  public static int computeScore(int typeA, int typeB) {
    System.out.println(typeA + " and " + typeB);
    System.out.println(typeInfo);
    int containedNumA = typeInfo.get(typeA-1);
    int containedNumB = typeInfo.get(typeB-1);
...
//    System.out.println(harmony);
  ...
  public static int DFS(int y, int x, int type, int cnt) {
//    area = new int[n][n];
    area[y][x] = type;
    for (int d = 0; d < dir.length; d++) {
      int ny = y + dir[d][0];
      ...
      if (board[y][x] == board[ny][nx]) {
        area[ny][nx] = type;
        visited[ny][nx] = true;
        cnt = Math.max(cnt, DFS(ny, nx, type, cnt + 1));
      }
    }
